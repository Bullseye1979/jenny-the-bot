...
export default async function getWebpageChannelConfig(coreData) {
  ...
  const cfg = coreData?.config?.["webpage-channel-config"];
  ...
  if (!channelId || !flow) return coreData;

  const anyChannelMatch = channels.some(ch => matchChannel(ch, channelId));
  if (!anyChannelMatch) return coreData;

  const { appliedKeys, matchedRules, warnings } = applyStrictHierarchy(
    workingObject, channels, channelId, flow, userId
  );

  workingObject.channelallowed = true;

  log(
    "Applied webpage channel hierarchy",
    warnings.length > 0 ? "warn" : "info",
    { moduleName: MODULE_NAME, channelId, userId: userId || "unknown", flow, matches: matchedRules.length, appliedKeys, warnings }
  );
