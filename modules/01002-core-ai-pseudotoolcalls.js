...
function getPromptFromSnapshot(rows, kiCfg) {
  ...
  for (let i = 0; i < rows.length; i++) {
    const r = rows[i] || {};
    if (r.role === "system") {
      if (kiCfg.includeHistorySystemMessages) out.push({ role: "system", content: r.content ?? "" });
    }
        ...
        if (toolCallsUsedTotal > 0) {
          ...
          try {
            ...
            if (_fbRes.ok) {
              ...
              if (_fbText) { accumulatedText = _fbText; log(`Partial fallback succeeded: ${_fbText.length} chars`, "info"); }
              else { log("Partial fallback returned empty", "warn"); }
            } else {
