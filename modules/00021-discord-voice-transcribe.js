/***************************************************************
/* filename: "00021-discord-voice-transcribe.js"               *
/* Version 1.0                                                 *
...
/* Creates a temporary directory and returns a file path       *
/***************************************************************/
...
/* Converts raw PCM stream to a WAV file using ffmpeg          *
/***************************************************************/
...
/* Converts a PCM Buffer to a WAV file                         *
/***************************************************************/
...
/* Computes simple SNR and voiced frames mask over PCM samples *
/***************************************************************/
...
/* Loads WAV, extracts PCM, analyzes frames                    *
/***************************************************************/
...
/* Builds a WAV file containing only voiced frames             *
/***************************************************************/
...
/* Resolves final OpenAI transcription endpoint URL            *
/***************************************************************/
...
/* Sends WAV to OpenAI Whisper HTTP API and returns text       *
/***************************************************************/
...
/***************************************************************
/* functionSignature: getCaptureOneSegment (receiver, userId,o)*/
/* Records one voice segment and writes it as WAV              *
/***************************************************************/
...
/* Captures, filters, transcribes Discord voice into payload   *
/***************************************************************/
export default async function getDiscordVoiceTranscribe(coreData) {
  ...
  if (!sessionKey || !userId) { log("Missing sessionKey or userId", "warn", { moduleName: MODULE_NAME, sessionKey, userId }); wo.stop = true; return coreData; }
  const live = getItem(sessionKey);
  const receiver = live?.connection?.receiver;
  if (!receiver) { log("No receiver on session", "error", { moduleName: MODULE_NAME, sessionKey }); wo.stop = true; return coreData; }
  ...
  try {
    ...
    if (!WHISPER_KEY) {
      for (const w of filteredWavs) if (!KEEP_WAV) { try { await fs.promises.rm(w.dir, { recursive: true, force: true }); } catch {} }
      wo.stop = true;
      return coreData;
    }
    const parts = [];
    ...
    if (finalText) wo.payload = finalText;
    try {
      const captureKey = `discord-voice:capture:${sessionKey}`;
      const prev = getItem(captureKey) || [];
      prev.push({
        ts: Date.now(),
        sessionKey,
        guildId: wo.guildId,
  ...
  if (!wo.payload || !String(wo.payload).trim()) {
    wo.stop = true;
  }
  return coreData;
}
