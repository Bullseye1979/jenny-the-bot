...
| "Generated by …" model footer missing from articles | The internal API response (`09300-webpage-output.js`) only returned `{ response }` — `wo.model` (set by `core-channel-config` for the wiki channel) was not included. `callPipelineForArticle` had no way to know which model was used, so `article._model` was always `null` and nothing was stored in `wiki_articles.model`. | Fixed: `09300` now returns `{ response, model }`. `callPipelineForArticle` reads `data.model` and sets `article._model`, which is stored in the DB and rendered at the bottom of the article page. Existing articles without a stored model show no footer until they are regenerated. |
