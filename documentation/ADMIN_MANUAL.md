...
3. Provides the latest assistant text first, followed by latest user input, visual source material from `imagePersonaHint`, `persona`, and `systemPrompt`, then recent context for continuity only. Pass 2 must extract only visible appearance from the visual source material.
...
**Purpose:** Instructs the user's Jenny browser extension to open a new tab with the given URL. The action is stored in the in-memory registry (30 s TTL) and delivered exactly once. A persistent offscreen document in the extension polls `/browser-action` every 5 seconds regardless of user interaction — the tab opens within 5 seconds of the command being issued, with neither a tab switch nor an open side panel required.
