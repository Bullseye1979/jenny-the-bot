...
- After the AI returns the article JSON, `callPipelineForArticle` reads `wo.model` (set by `core-channel-config` for the wiki channel) and stores it in `article._model`, which is persisted to `wiki_articles.model` and shown at the bottom of the article page as "Generated by …".
...
| "Generated by …" model footer missing from articles | `callPipelineForArticle` never read `wo.model` after the AI returned the article JSON, so `article._model` was always `null` and nothing was stored in `wiki_articles.model`. | Fixed: `callPipelineForArticle` now reads `wo.model` (set by `core-channel-config` for the wiki channel) after parsing the article and sets `article._model` → stored in `wiki_articles.model` → shown in article footer. |
