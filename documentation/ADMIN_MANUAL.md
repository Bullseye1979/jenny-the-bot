...
- **Image generation** is mandatory per article (`getImage` is a required step in the AI prompt). AI-generated images → `pub/documents/wiki/` (isolated subdirectory, safe to delete); uploaded images → `pub/wiki/{channelId}/images/`. Both locations are served via `09300-webpage-output.js` under `/documents/wiki/` and `/wiki/{channelId}/images/`. Requires `toolsconfig.getImage.publicBaseUrl` to be configured.
