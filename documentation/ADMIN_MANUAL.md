...
- The internal API response includes `{ response, model }`. `model` is `wo.model` as set by `core-channel-config` for the wiki channel. `callPipelineForArticle` stores this in `article._model`, which is persisted to `wiki_articles.model` and shown at the bottom of the article page as "Generated by …".
...
| "Generated by …" model footer missing from articles | `09300` only returned `{ response }` — `wo.model` (set by `core-channel-config` for the wiki channel) was not included. `callPipelineForArticle` had no model to store. | Fixed: `09300` now returns `{ response, model: wo.model }`. `callPipelineForArticle` reads `data.model` → `article._model` → stored in `wiki_articles.model` → shown in the article footer. |
