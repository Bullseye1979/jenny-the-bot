...
- After the AI returns the article JSON, `callPipelineForArticle` reads the model directly from `core.json["core-channel-config"].channels[].overrides.model` for the wiki channel and stores it in `article._model`, which is persisted to `wiki_articles.model` and shown at the bottom of the article page as "Generated by …". `wo.model` from neither the outer request nor the inner pipeline can be used reliably — for `/api` calls the body-parser sets `wo.channelID` after `core-channel-config` has already run, so the channel's model override never reaches `wo.model` in the inner pipeline.
...
| "Generated by …" model footer missing from articles | Neither the outer `wo.model` nor the inner pipeline's `wo.model` reliably carry the wiki channel's model — `core-channel-config` runs before the body-parser sets `wo.channelID` for `/api` calls. | Fixed: `callPipelineForArticle` reads the model directly from `coreData.config["core-channel-config"].channels[].overrides.model` for the wiki `channelId` — bypassing the pipeline execution order entirely. |
