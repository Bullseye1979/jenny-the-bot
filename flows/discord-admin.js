...
/* Version 1.0                                                *
/* Purpose: Flow trigger for Discord slash commands.           *
/*          Registers commands from workingObject, snapshots   *
/*          interactions safely, enforces admin gate, and      *
/*          forwards to the "discord-admin" flow silently.     *
/**************************************************************/
/**************************************************************
/*                                                            *
/**************************************************************/
...
function getCommandDefinitions(root, log) {
  ...
  log?.("no workingObject.discord-admin.slash.definitions → registering fallback /avatar", "info", { moduleName: "discord-admin" });
...
/**************************************************************
/* functionSignature: getCheckAllowByAdminArray (defLike, uid) *
...
/**************************************************************
/* functionSignature: setEnsureGuildRegistration (client,d,l)  *
    ...
    try {
      ...
      if (missing.length > 0) {
        try {
          ...
          log(`Repaired slash registrations for guild ${gid}`, "info", { moduleName: "discord-admin", repaired: missing.map(m => m.name) });
        } catch {}
      }
    } catch {}
  }
...
/**************************************************************
/* functionSignature: getBuildRunCore (make, i, baseWO)        *
...
/**************************************************************
/* functionSignature: setStartWhenClientAvailable (base,run,make) *
...
async function setStartWhenClientAvailable(baseCore, runFlow, createRunCore) {
  ...
  async function tryStart() {
    try {
      ...
      if (changed) {
        log("slash commands registered/updated", "info", { moduleName: "discord-admin", count: defs.length });
      }
      ...
      ticking = false;
      log("discord-admin ready", "info", { moduleName: "discord-admin" });
    } catch {}
  }
  await tryStart();
  if (!ticking) return;
...
/**************************************************************
/* functionSignature: getDiscordAdminFlow (base, run, make)    *
