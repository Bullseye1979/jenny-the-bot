...
async function setAttachToSession(baseCore, sessionKey, session, cfg, runFlow, createRunCore, log) {
  ...
  if (!receiver) {
    log("missing receiver", "error", { moduleName: MODULE_NAME, sessionKey });
    return;
  }
  ...
  const voiceChannelId =
    ...
    (session.channelId && session.channelIdType === "voice" ? session.channelId : null) ??
    null;

  const textChannelId =
    session.textChannelId ??
    (session.message && session.message.channelId) ??
    (session.channelId && session.channelIdType === "text" ? session.channelId : null) ??
    null;
  ...
  receiver.speaking?.on?.("start", async (uidLike) => {
    try {
      ...
      if (typeof uidLike === "string" || typeof uidLike === "number") {
        userId = String(uidLike);
      } else if (uidLike && typeof uidLike === "object") {
        userId = uidLike.id || uidLike.userId || uidLike.user_id || null;
      }
      ...
      if (client && guildId) {
        try {
          speaker = await getResolveSpeakerName(client, guildId, userId);
        } catch {}
      }
