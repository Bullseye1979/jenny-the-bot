...
/* Version 1.0                                                 *
/* Purpose: Poll registry, attach voice speaking triggers, and *
/*          forward captures into the "discord-voice" flow     *
/* Notes:   Respects separate textChannelId vs voiceChannelId  *
/*          written by /join module (no legacy confusion).     *
/***************************************************************/
/***************************************************************
/*                                                             *
/***************************************************************/
...
async function setAttachToSession(baseCore, sessionKey, session, cfg, runFlow, createRunCore, log) {
  ...
  receiver.speaking?.on?.("start", async (uidLike) => {
    try {
      ...
      const targetId = useVoice ? (voiceChannelId || textChannelId || null)
